# Local vs Remote 评审:不付双倍钱的架构指南(v2.6.3)

> 一个常被忽略的问题:**reusable workflow 在 GitHub Actions 里跑,要不要再买一套云上 LLM API key?**
>
> 答:v2.6.3 起默认用 Codex CLI 跑评审。Pro/个人版远端 CI 用 `OPENAI_API_KEY`;Business/Enterprise workspace 可以用官方 `CODEX_ACCESS_TOKEN`。若你只是单人本地用,仍然完全可以不配远端 secret。
...
─────────────────────             ─────────────────────────
认证:  你的本地登录态 / 订阅        认证:  *它自己的* OPENAI_API_KEY 或 CODEX_ACCESS_TOKEN
                                       (它认不出你本机登录态)
触发:  你手动 prompt               触发:  *任何人* 开 PR / push
位置:  你的本地 fs                 位置:  GitHub-hosted runner
```

**核心约束**:GitHub Actions 跑在远端 runner 上,**不认识你本机 Codex session**。所以它要做评审,必须有自己能用的 CI secret。
...
| **B. Codex CLI in CI**(reusable workflow 默认) | ✅ `OPENAI_API_KEY` 或 `CODEX_ACCESS_TOKEN` | 全自动 | Pro 用 Platform API 账单;Business/Enterprise 看组织额度 | 多人协作、要 required check、要审计、跨设备 |
...
## B. Codex CLI in CI(reusable workflow 默认)

**怎么做**:见 [`reusable-workflows.md`](reusable-workflows.md) — 几行 `uses:` + 配 GitHub Secret。Pro/个人版用 `OPENAI_API_KEY`;Business/Enterprise 可用 `CODEX_ACCESS_TOKEN`。

**好处**:全自动、required check 成立、跨设备一致、有审计 log。
**代价**:Pro/个人版会走 OpenAI Platform API 计费;secret 只适合受信任仓库/受信任 workflow。
...
| 我开始有同事提 PR / 想 required check | **B**(Pro 配 `OPENAI_API_KEY`;Business/Enterprise 可配 `CODEX_ACCESS_TOKEN`) |
...
- ❌ 把浏览器登录态 / ChatGPT cookie 当 CI secret 用 —— 协议不一样,会失败
...
从 Local-only 升到 Codex CLI in CI 不是回头路——是**渐进的**:

```
Day 0   纯本地评审,免费验证 harness 价值
Day N   团队增加到 2 人 / 你开始上 CI required check
       ↓
       开 PR 配 GitHub Secret:OPENAI_API_KEY(Pro) 或 CODEX_ACCESS_TOKEN(Business/Enterprise)
