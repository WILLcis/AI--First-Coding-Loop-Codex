...
- [ ] **LLM API Key**:OpenAI 或任意兼容厂商(Anthropic / DeepSeek / Qwen / Kimi / GLM 等),准备好 `LLM_PROVIDER` 与 `LLM_API_KEY`
...
### 2.1 LLM provider + 三趟 AI 评审

- [ ] 仓库 → **Settings → Secrets and variables → Actions → New repository secret**
  - `LLM_API_KEY` = 你选择的模型厂商 API key
  - Variable:`LLM_PROVIDER` = `openai` / `anthropic` / `deepseek` / `qwen` / `kimi` / `glm` / `custom`
  - Variable:`LLM_MODEL` = 默认模型字符串
