# 接力包:把进行中的项目升级到 Codex v2.5 harness
...
```bash
git switch -c chore/upgrade-codex-harness-v2.5
rm -rf /tmp/aifcl
...
- `AGENTS.md`:保留项目真实命令、目录、约束;只追加 v2.4 coding discipline 段落
...
如果用户是单人开发、测试期、或暂时不需要 required check,优先启用 v2.5 local-first:
...
git commit -m "chore: upgrade Codex harness to v2.5"
git push -u origin chore/upgrade-codex-harness-v2.5
gh pr create --draft \
  --title "[harness] upgrade Codex harness to v2.5" \
...
2. secret-scan 连续绿后设为 required
3. image-scan 噪音处理完后设为 required
4. perf-gate 写真实 scenario + 建 baseline 后再设 required
5. 单独 PR 清理旧 `.claude/` / `CLAUDE.md` 路径(如有)
```
...
```text
请读取 handoffs/handoff-upgrade-existing-project-to-codex-v2.5.md,
